  ...
  return (
    <motion.div layout className="mb-4">
      <form action={sendMessage} ref={ref}>
        <div className="mt-2 grid w-full flex-shrink-0 grid-cols-6 items-center justify-stretch justify-items-center rounded-3xl bg-background-text">
          {isAuthenticated ? <PanelDrawer /> : null}
          <input
            name="user_prompt"
            placeholder="What would you like to know..."
            className="col-span-4 col-start-2 m-3 w-full bg-background-text text-xl text-secondary placeholder:bg-background-text focus:outline-none"
            autoComplete="off"
            disabled={isPending}
          ></input>
          {!isAuthenticated ? (
